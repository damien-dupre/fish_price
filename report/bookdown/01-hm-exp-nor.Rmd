# Horse mackerel exports from Norway
...
ggplot() +
  geom_sf(data = world) +
  geom_sf(data = data_hm_exp_world, aes(fill = n_price)) +
  #geom_sf(data = data_hm_exp_world, aes(geometry = centroid, size = m_price)) +
  stat_sf_coordinates(data = data_hm_exp_world, aes(color = sd_price), geom = "point") +
  scale_x_continuous("") +
  scale_y_continuous("") +
...
The trend of the evolution of fish price since `r min(data_hm_exp_raw$year)` indicates follows a slight parabolic inverse trend with a valley (negative peak) in 2016.
...
data_hm_exp_ti %>%
  ...
  annotate("text", x = ymd("2014-01-01"), y = 2.5, label = "Train Region") +
  annotate("text", x = ymd("2019-01-01"), y = 2.5, label = "Test Region") +
  annotate("text", x = ymd("2021-03-01"), y = 2.5, label = "Forecast Region") +
  geom_point(alpha = 0.25) + 
  #geom_point(aes(x = time, y = .pred), data = prediction_tbl, alpha = 0.5, color = "red") +
...
However the results show that the number of trade has no significant relationship with the average unit price (`r data_hm_exp_lm$full_result$modelfit$r2`).
...
It seems logical to believe that countries buying more will have more leverage to negociate their unit price. The results reveals a weak but significant relationship between the unit quantity ordered on a log scale and unit price (`r data_hm_exp_lm$full_result$modelfit$r2`).
...
```{r}
list_country_keep <- data_hm_exp_summary %>% 
  filter(n_price > 10) %>% 
